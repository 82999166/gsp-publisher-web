...
## 发布排版问题修复 - 新方案（2026-05-07）

- [x] 放弃使用 Google Sites 的标题快捷键（Ctrl+Alt+2/3 不起作用）
- [x] 改用视觉分隔符方案：
  - H1 标题：用 `█` 符号标记
  - H2 标题：用 `═` 符号标记
  - H3 标题：用 `─` 符号标记
...
## 发布问题修复 - 第二阶段（2026-05-07）

- [x] 修复网站名称不显示问题 - 在发布弹窗中正确填写网站名称
- [x] 修复内嵌网站不显示问题 - 从内容中提取 iframe 并正常插入
- [ ] 测试修复后的发布效果


## 文章生成流程一致性检查（2026-05-07）

- [x] 分析单条文章生成流程的所有参数和模板
- [x] 分析批量文章生成流程的所有参数和模板
- [x] 检查发布样式是否一致
- [x] 统一参数处理逻辑 - 所有 API 都支持相同的参数集合
- [x] 统一模板应用方式 - 所有 API 都支持模板 ID
- [x] 统一发布样式 - 质量分、自动审核、元数据计算统一
- [ ] 测试单条和批量生成的一致性


## SEO 模板调用问题修复（2026-05-07）

- [x] 修复批量生成配置中的 SEO 模板选择器 - 应该显示可选的模板列表
- [x] 修复后端 API 参数传递 - 确保 templateId 被正确传递
- [ ] 测试 SEO 模板选择和生成流程
